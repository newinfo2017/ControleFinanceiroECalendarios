...
# Função para abrir a janela pop-up de consulta de reuniões
def consultar_reunioes():
    popup = tk.Toplevel()
    popup.title("Reuniões Agendadas")
    
    if not reunioes:
        tk.Label(popup, text="Nenhuma reunião agendada.").pack(padx=20, pady=20)
    else:
        texto_consulta = tk.Text(popup, height=15, width=50)
        texto_consulta.pack(padx=10, pady=10)
        for reuniao in reunioes:
            texto_consulta.insert(tk.END, f"Título: {reuniao['titulo']}\n")
            texto_consulta.insert(tk.END, f"Data e Hora: {reuniao['data_hora'].strftime('%d/%m/%Y %H:%M')}\n")
            texto_consulta.insert(tk.END, f"E-mail: {reuniao['email']}\n")
            texto_consulta.insert(tk.END, f"WhatsApp: {reuniao['whatsapp']}\n")
            texto_consulta.insert(tk.END, "-"*30 + "\n")
        texto_consulta.config(state=tk.DISABLED)  # Desativar edição do texto
...
botao_consultar = tk.Button(janela, text="Consultar Reuniões", command=consultar_reunioes)
botao_consultar.grid(row=6, column=0, columnspan=2, pady=10)
